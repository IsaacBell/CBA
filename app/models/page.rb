...
class Page < ActiveRecord::Base
  ...
  mount_uploader :desc_bg_image, ImageUploader

  scope :published, lambda { where(:status => true) }
  scope :unpublished, lambda { where(:status => false) }
  scope :navlinked, lambda { where(:show_in_nav => true) }
  ...
  scope :masters, lambda { where(["slug LIKE ? OR slug LIKE ?", "master","masters"]) }


  enum bgstyle: { "text-center" => 0, "bg-light" => 1, "bg-dark" => 2, "bg-primary" => 3, "bg-alt" => 4, "bg-img" => 5, "bg-parallax" => 6,"bg-parallax bg-transparent-light" => 7, "bg-parallax bg-transparent-dark" => 8, "bg-parallax bg-transparent-primary" => 9 }
